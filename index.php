...
if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['emailid'])) {
    ...
    try {
        ...
        error_log("LOGIN ATTEMPT - Email: $email, Vercode: $vercode, Session: " . $_SESSION["vercode"]);
        ...
        if (empty($email) || empty($password)) {
            echo json_encode(array(
                'type' => 'error',
                'title' => 'ข้อมูลไม่ครบ',
                'message' => '<i class="fa fa-exclamation-circle me-2"></i>กรุณากรอกข้อมูลทั้งหมด',
                'redirect' => null
            ));
            exit;
        }
        ...
        $sql = "SELECT Email, Password, id, Role, Status FROM tblmembers WHERE Email=:email AND Password=:password AND Role IN ('student','admin')";
        ...
        if($query->rowCount() > 0) {
            ...
            if((int)$result->Status !== 1) {
                error_log("LOGIN BLOCKED - User banned: Email: $email, Status: " . $result->Status);
                echo json_encode(array(
                    'type' => 'warning',
                    'title' => 'บัญชีถูกแบน ❌',
            ...
            error_log("LOGIN SUCCESS - Email: $email, ID: " . $result->id);
            
            echo json_encode(array(
                'type' => 'success',
                'title' => 'เข้าสู่ระบบสำเร็จ',
                'message' => '<i class="fa fa-check-circle me-2"></i>ยินดีต้อนรับกลับมา',
            ...
            echo json_encode(array(
                ...
                'message' => '<i class="fa fa-exclamation-circle me-2"></i>อีเมลหรือรหัสผ่านไม่ถูกต้อง',
                'redirect' => null
            ));
        }
        exit;
...
<head>
    ...
    <meta name="description" content="Sports Equipment Booking System" />
    <meta name="author" content="" />
    <title>E-Sports | Login</title>
    <?php
    // Dynamically set base href to support subfolder deployments like /E-Sports/
    $basePath = dirname($_SERVER['SCRIPT_NAME']);
    $baseHref = ($basePath === DIRECTORY_SEPARATOR || $basePath === '\\' || $basePath === '.' || $basePath === '')
        ? '/'
        : rtrim($basePath, '/\\') . '/';
    ?>
    ...
    <script type="text/javascript">
        // Resolve base path robustly for subfolder deployments
...
<body>
                    ...
                    <div class="col-md-5 col-lg-4">
                        <div class="auth-card">
                            <div class="auth-card-header">
                                <h2>
                                    <i class="fa fa-sign-in me-3"></i>เข้าสู่ระบบ
                                </h2>
                            </div>

                            <div class="auth-card-body">
                                <form id="loginForm" method="post" data-no-auto-loading="1" onsubmit="return submitLoginForm(event)" novalidate>
                                    ...
                                    <div class="auth-form-group">
                                        ...
                                        <small class="text-muted d-block mt-1">
                                            <a href="user-forgot-password.php" class="text-decoration-none">ลืมรหัสผ่าน?</a>
                                        </small>
                                    </div>
                                    ...
                                    <button type="submit" name="login" class="btn btn-primary btn-lg auth-submit-btn" id="submit">
                                        <i class="fa fa-sign-in me-2"></i>เข้าสู่ระบบ
                                    </button>
                                    ...
                                    <p class="auth-footer-text">
                                        ยังไม่มีบัญชี? <a href="signup.php" class="fw-600 text-decoration-none">ลงทะเบียนตอนนี้</a>
                                    </p>
                                </form>
                            </div>
                        </div>
    ...
    <script>
        ...
        function refreshCaptcha() {
            ...
            const url = (BASE_PATH ? BASE_PATH : '') + '/captcha.php?t=' + timestamp;
            captchaImg.src = url;
        ...
        function submitLoginForm(event) {
            ...
            const defaultHTML = submitBtn.getAttribute('data-default-html') || '<i class="fa fa-sign-in me-2"></i>เข้าสู่ระบบ';
            ...
            const postUrl = (BASE_PATH ? BASE_PATH : '') + '/index.php';
            fetch(postUrl, {
                method: 'POST',
                body: formData,
            ...
            .then(data => {
                ...
                if (data.type === 'success') {
                    keepLoading = true; // Keep spinner only for success path
