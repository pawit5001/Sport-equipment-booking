...
$bookingId = isset($_GET['id']) ? (int)$_GET['id'] : ($_SESSION['last_booking_id'] ?? 0);
...
foreach ($items as &$item) {
    ...
    // คำนวณค่าปรับถ้ายังไม่คืนและเกินกำหนด
    if ($item['DueDate'] && $item['ReturnStatus'] != 1 && ($item['FineAmount'] ?? 0) == 0) {
        $dueDate = new DateTime($item['DueDate']);
        if ($today > $dueDate) {
            $overdueDays = $dueDate->diff($today)->days;
            $calculatedFine = $overdueDays * ($item['FinePerDay'] ?? 0) * $item['Quantity'];
...
<body>
    <!-- Action Buttons -->
    <div class="container no-print" style="max-width: 800px; margin-top: 20px;">
        <div class="action-buttons">
            <a href="my-bookings.php" class="btn btn-outline-secondary">
                <i class="fa fa-arrow-left me-1"></i> กลับ
            </a>
            <button onclick="window.print()" class="btn btn-primary">
                <i class="fa fa-print me-1"></i> พิมพ์ใบเสร็จ
            </button>
            <a href="dashboard.php" class="btn btn-outline-primary">
                <i class="fa fa-home me-1"></i> หน้าแรก
            </a>
        </div>
    </div>
    ...
    <div class="receipt">
        ...
        <div class="receipt-body">
            ...
            <div class="footer-note">
                ...
                <p>กรุณาคืนอุปกรณ์ตามวันกำหนด หากคืนล่าช้าจะมีค่าปรับตามอัตราที่ระบุ</p>
